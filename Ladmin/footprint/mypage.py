...
#将动态方法转为静态属性
    ...
    def page_html(self):
        ...
        # 加上第一页
        self.params["page"]=1
        html_str_list.append('<li><a href="{0}?{1}">首页</a></li>'.format( self.url_prefix,self.params.urlencode()))
        # 判断一下 如果是第一页，就没有上一页
        if self.page_num <= 1:
            ...
            html_str_list.append('<li><a href="{}?{}"><span aria-hidden="true">&laquo;</span></a></li>'.format( self.url_prefix,self.params.urlencode()))
        ...
        if self.page_num >= self.total_page:
            ...
            self.params["page"] = self.page_num +1
            html_str_list.append('<li><a href="{0}?{1}"><span aria-hidden="true">&raquo;</span></a></li>'.format( self.url_prefix,self.params.urlencode()))
        # 加最后一页
        self.params["page"] = self.total_page
        html_str_list.append('<li><a href="{0}?{1}">尾页</a></li>'.format( self.url_prefix, self.params.urlencode()))
